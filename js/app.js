...
function goHome(){// 投资首页
    ...
    var currRadio1 = storeUtil.get(config.keyPreWord+config.radio);

    if(currRadio1 != undefined && currRadio1 != null){// 已经设置了频道
        if(homeFlag == 0){
            homeFlag = 1;

            currRadio = currRadio1;
            goLoanHome();
        }
    }else{// 没有设置频道
        show2("myradio");
        ...
        currRadio = 0;
        storeUtil.set(config.keyPreWord+config.radio,currRadio);// 保存在本地存储中

        //setRadio(0);// 默认设置为女频道
        hide("startUp");

        /*
        setTimeout(function () {// 不点击自动进入
            goHome();
        }, 1000 * 3);*/
    }
...
function goInvestHome(){// 书架界面
    ...
    $.requestData(2,service.getShelfList,$.toJasonParams(data),function(result){// 提交服务器
        ...
        if(result.stars == 1){
            ...
            var otherdata = result.data.otherData;
